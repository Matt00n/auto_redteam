...
def main():
    ...
    code_critic = CodeCritic(
        llm=llm, editor_id=editor_id, required_chars=required_chars
    )
    ...
    for i in range(ITERATIONS):
        ...
        available_modes = ["choose", "sample", "unbound", "random"]
        if unused_seeds:
        ...
        for attempt in range(3):
            print(
                f"[*] Mastermind is generating a hypothesis (Attempt {attempt + 1})..."
            )
        ...
        for attempt in range(3):
            print(
                f"[*] Hacker is writing the {execution_mode} exploit script (Attempt {attempt + 1})..."
            )
            ...
            # Syntax is OK, now run LLM Code Review using Code Critic
            print("[*] Code Critic reviewing script...")
            critique = code_critic.evaluate(exploit_code, hypothesis_raw)
            if critique.get("approved", False):
                print("[+] Code Approved by Critic!")
                break
            else:
                code_feedback = critique.get("feedback", "Rejected.")
                print(f"[-] Code Rejected by Critic: {code_feedback}")
                if attempt == 2:
        ...
        for attempt in range(max_reset_attempts):
            try:
                ...
                print(
                    f"[!] Warning: Target state reset attempt {attempt + 1} failed: {e}"
                )
        ...
        if execution_mode in ("automated", "computer_use", "direct_websocket"):
            evidence = executor.execute_python_code(
                exploit_code, run_id=run_id, timeout_seconds=30
            )
            ...
            if evidence["return_code"] != 0:
                outcome_notes = (
                    f"Script execution failed. Stderr: {evidence['stderr'][:200]}..."
                )
                outcome_success = False
                outcome_score = 0.0
            else:
                outcome_notes = (
                    "Script executed successfully. Awaiting Judge validation."
                )
                outcome_success = False
                outcome_score = 0.0
            ...
            if evidence.get("return_code") == 0:
                judge_report = judge.evaluate_success(
                    assignment_taker_id, threshold=required_chars
                )
        ...
        if len(human_tasks_queue) >= 3 or (
            ...
            for task in human_tasks_queue:
                ...
                user_feedback = input(
                    "Did this work? (Type 'y' for success, or provide failure notes): "
                ).strip()
